...
fun main() = with(System.`in`.bufferedReader()) {
    ...
    visited = Array(2) { IntArray(2 * n) { 0 } }
    bfs(track)
    visited.forEach {
    ...
    for (row in 0 until 2) {
        if (visited[row][n - 1] != 0)
            track1 = min(track1, visited[row][n - 1] - 1)
    }
    println(track1)
    if (track1 == INF) {
        println(-1)
    ...
    for (row in 0 until 2) {
        if (visited[row][n - 1] != 0)
            track2 = min(track2, visited[row][2*n - 1] - 1)
    }
    println(track2)
    if (track2 == INF) {
        println(-1)
